...
int get_slice_num(int num, int thread_id, int r_val);
...
void search_by_thread( void* ptr)
{
	...
	int i=0;
	int j;
	while( (temp_slice = get_slice_num(data->num_slices, data->thread_id, 0)) != -1)
	{
		j =0;
		...
		printf("THREAD NUM(#%d) SLICE NUM(#%d) COUNT(#%d)\n",data->thread_id, temp_slice, j); 
		i++;
	}
	printf("THREAD (%d) RAN -->%d<--\n", data->thread_id, i);
	pthread_exit(0);
}


int get_slice_num(int num_slices, int thread_id, int r_val)
{
